...
package com.handy.widget.spinner;
...
public class MaterialSpinner extends android.support.v7.widget.AppCompatTextView {
    ...
    private void init(Context context, AttributeSet attrs) {
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.HDWMaterialSpinner);
        int defaultColor = getTextColors().getDefaultColor();
        boolean rtl = Utils.isRtl(context);

        try {
            backgroundColor = ta.getColor(R.styleable.HDWMaterialSpinner_hdw_spinner_background_color, Color.WHITE);
            textColor = ta.getColor(R.styleable.HDWMaterialSpinner_hdw_spinner_text_color, defaultColor);
            arrowColor = ta.getColor(R.styleable.HDWMaterialSpinner_hdw_spinner_arrow_tint, textColor);
            hideArrow = ta.getBoolean(R.styleable.HDWMaterialSpinner_hdw_spinner_hide_arrow, false);
            popupWindowMaxHeight = ta.getDimensionPixelSize(R.styleable.HDWMaterialSpinner_hdw_spinner_drop_maxheight, 0);
            popupWindowHeight = ta.getLayoutDimension(R.styleable.HDWMaterialSpinner_hdw_spinner_drop_height, WindowManager.LayoutParams.WRAP_CONTENT);
        ...
        if (getBackground() == null) {
            setBackgroundResource(R.drawable.hdw_spinner_selector);
        }
        ...
        if (!hideArrow) {
            arrowDrawable = Utils.getDrawable(context, R.drawable.hdw_spinner_arrow).mutate();
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            popupWindow.setBackgroundDrawable(Utils.getDrawable(context, R.drawable.hdw_spinner_drawable));
        } else {
            popupWindow.setBackgroundDrawable(Utils.getDrawable(context, R.drawable.hdw_spinner_drop_shadow));
        }
...
//                Log.e("MaterialSpinner", "Error setting background color", e);
    ...
    private int calculatePopupWindowHeight() {
        ...
        float listViewHeight = adapter.getCount() * getResources().getDimension(R.dimen.hdw_spinner_item_height);
